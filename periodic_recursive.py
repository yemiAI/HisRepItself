...
def get_best(anim, lookaheadmin = 60, lookaheadmax = 100):

    per_frame_intervals = []
    best_val = {'from': None, 'to': None, 'difference': float('inf')}  # Initialize best_val with infinite difference
    for frame_from in range(anim.shape[0] - lookaheadmax):
        frame_to = frame_from + lookaheadmin + np.argmin(
            np.linalg.norm(anim[frame_from] - anim[frame_from + lookaheadmin:frame_from + lookaheadmax], axis=1))
        diff = np.linalg.norm(anim[frame_from] - anim[frame_to])
    ...
    print("Best is ", best_val)
    return best_val


def recurse(anim, lookaheadmin = 60, lookaheadmax = 100):
    best_vals = []

    if (anim.shape[0] < lookaheadmax):
        print("Shape return: %d"%anim.shape[0])
        return best_vals

    best_val = get_best(anim)
    if (best_val['from'] is None):
        print("No best found")
        return best_vals

    best_vals.append(best_val)

    best_vals.extend(recurse(anim[:lookaheadmin], lookaheadmin, lookaheadmax))
    best_vals.extend(recurse(anim[lookaheadmax:], lookaheadmin, lookaheadmax))
    return best_vals
...
if (__name__ == '__main__'):
    ...
    best_val = get_best(loader.rawvals)

    best_vals = recurse(loader.rawvals)
    #print(best_vals)
    #exit(0)
    ...
    filename = "datasets/recursive_synthetic/S11/walkingtogether_2.txt"
